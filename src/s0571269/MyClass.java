...
//		return getAtan2(p.x, p.y, info.getX(), info.getY());
//	}
//
...
//		// to types of check (1)contain(to check if it's two points from same obs)
//		// (2)intersect (to see if there's other obs in between)
//
...
//
//		// get two vectors from j to j-1 and j to j+1, get the smaller angle if middle
//		// to smaller angle is contained or intersect the obstacle, it's reflexive
//
...
//		// float result = (float) (Math.acos( (a+b-c) / Math.sqrt(4*a*b) ) *
//		// 180/Math.PI);
//		// //only returns 0-180
//		// if(debug)
...
//		getShortestWay();
//
//		angularAcc = turnToCheckpoint(info.getCurrentCheckpoint());
//		acceleration = getThrottle(info.getCurrentCheckpoint());
//		for (int i = 0; i < obstacles.length; i++) {
...
//			if (distance(null, p) < 20 && listPointCounter < shortList.size() - 1
//					&& !p.equals(info.getCurrentCheckpoint())) {
//				System.out.println("!!!!!!!!!!");
//				listPointCounter++;
//			}
//
...
//			System.out.println("-----dist: " + distance(null, p) + "-i:" + listPointCounter + "---shlistsize:"
//					+ shortList.size() + "-----equals: " + p.equals(info.getCurrentCheckpoint()) + "---");
//
//		}
//
//	}
...
//		getShortest = true;
//		g = new Graph(new Point((int) info.getX(), (int) info.getY()), info.getCurrentCheckpoint(), 10);
...
//			}
//		}
//		// System.out.println("goal_ori: " + get_ori(info.getCurrentCheckpoint()) + "
//		// currentDirectoion: " + currentDirection+" angleDIff: "+angleDifference);
//
