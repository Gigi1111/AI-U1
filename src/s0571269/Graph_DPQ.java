...
public class Graph_DPQ {
	...
	private Map<Point, List<Edge>> adjVertices = new HashMap<Point, List<Edge>>();
	private List<Point> shortList = null;
	private Point[][] obsPlusBuffer;
	private Polygon[] obstacles;
	private Track track;
	private int buffer = 10;// buffer to nodes 10-13?
	...
	Point pointPlusBuffer(Polygon ob, int i, int j, int buffer) {
		...
		if (cross > 0 && xPrev != 0 && yPrev != 0 && xPrev != track.getWidth() && yPrev != track.getHeight()
				&& xNow != 0 && yNow != 0 && xNow != track.getWidth() && yNow != track.getHeight() && xNext != 0
				&& yNext != 0 && xNext != track.getWidth() && yNext != track.getHeight()) {
